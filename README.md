...
- `spread_stress_multipliers` と `latency_ms_grid` で spread拡大 / latency悪化のstress結果を `stress_results.csv` / JSON に保存します。`required_stress_spread_multiplier` と `required_stress_latency_ms` の条件は promotion gate の必須判定に使います。
...
strategy:
  fx_scalping:
    ...
    require_validation_gate_passed_for_promotion: true
    walk_forward_enabled: true
    min_walk_forward_folds: 3
    candidate_model_dir: models/fx_scalping/candidates
    ...
    required_stress_spread_multiplier: 1.5
    required_stress_latency_ms: 500
    min_walk_forward_pass_ratio: 0.6
    fail_closed_on_promotion_fail: true
    calibration_report_enabled: true
    outcome_store_enabled: true
    outcome_store_dir: data/scalping_outcomes
    outcome_store_format: parquet
